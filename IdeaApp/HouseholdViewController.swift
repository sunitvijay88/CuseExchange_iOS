...
class HouseholdViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int{
        return AdStore.householdItems.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let ad = AdStore.householdItems[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowHouseholdItem" {
            if let row = tableView.indexPathForSelectedRow?.row {
                let ad = AdStore.householdItems [row]
                
                let detailController = segue.destination as! HouseholdDetailViewController
