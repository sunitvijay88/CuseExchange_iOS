...
class HousingViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int{
        return AdStore.housingItems.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let ad = AdStore.housingItems[indexPath.row]
        
        cell.nameLabel.text = ad.name
        cell.serialNumberLabel.text = ad.desc
        cell.valueLabel.text = "$\(ad.valueInDollars)"
       //cell.imageTest?.image = self.imageStore.imageForKey(item.itemKey)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowHousingItem" {
            if let row = tableView.indexPathForSelectedRow?.row {
                let ad = AdStore.housingItems [row]
                
                let detailController = segue.destination as! HousingDetailViewController
