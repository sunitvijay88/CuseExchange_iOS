...
//  BooksDetail
//
//  Created by HEERA ANIL on 11/25/16.
...
class BooksDetailViewController: UIViewController, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        ref2.observeSingleEvent(of: .value, with: { snapshot in
            ...
            let messageSeller = UIAlertAction(title: "Message Seller",
                ...
                handler: { (action) -> Void in
                    //self.startNewGame()
                if let url = NSURL(string: "sms://\(phNum!)"), UIApplication.shared.canOpenURL(url as URL) {
            ...
            let callSeller = UIAlertAction(title: "Call Seller",
                style: .default ,
                handler: { (action) -> Void in
            if let url = NSURL(string: "tel://\(phNum!)"), UIApplication.shared.canOpenURL(url as URL) {
                        UIApplication.shared.openURL(url as URL)
                    }
            })
            ...
            let emailSeller = UIAlertAction(title: "Email Seller",
                style: .default ,
                handler: { (action) -> Void in
                    if let url = NSURL(string: "mailto://\(self.ad.userEmail)"), UIApplication.shared.canOpenURL(url as URL) {
                        UIApplication.shared.openURL(url as URL)
                    }
